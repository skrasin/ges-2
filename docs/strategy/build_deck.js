...
// Изображение с собственными пропорциями (скриншоты)
function picAR(slide, x, y, w, file, ar) {
  const h = w / ar;
  slide.addImage({ path: IMG + file, x, y, w, h });
  slide.addShape(pres.ShapeType.rect, {
    x, y, w, h, fill: { type: 'none' }, line: { color: INK, width: 0.5 },
...
// ============ 9. Цифровые сервисы ============
{
  ...
  picAR(s, 6.4, 2.6, 6.23, 'site.jpg', 2.0661);
  s.addNotes('Главная страница ges-2.org. Две точки трения видны прямо на экране: бегущая строка с режимом работы, которую нельзя перечитать, и кнопка «Регистрация на вход» без визуального приоритета, хотя это главное целевое действие посетителя.');
}
