...
function photo(slide, x, y, w, h, caption) {
  ...
  slide.addText(caption, {
    ...
    fontFace: FONT, fontSize: 10, color: MUTED, align: 'center', valign: 'middle',
...
function rule(slide, x, y, w) {
  slide.addShape(pres.ShapeType.line, {
    x, y, w, h: 0, line: { color: HAIR, width: 0.75 },
  });
}
...
// ============ 2. Наше понимание задачи ============
{
  ...
  st.forEach((t, i) => {
    s.addText(t, {
      x: ML, y, w: 7.0, h: 1.0,
      fontFace: FONT, fontSize: 19, color: INK, charSpacing: 0.38,
...
{
  const s = S();
  title(s, 'С кем работаем вместе');
  s.addText('Впечатление посетителя/клиента единое — складывается из работы нескольких команд', {
    x: ML, y: 1.42, w: CW, h: 0.4,
  ...
  pairs.forEach((p, i) => {
    ...
    const y = 2.2 + row * 1.5;
    s.addText([
      { text: p[0], options: { bold: true, breakLine: true } },
      { text: p[1], options: { fontSize: 13, color: '333333' } },
    ], {
      x: colX[col], y, w: colW, h: 1.3,
      fontFace: FONT, fontSize: 15, color: INK, margin: 0, isTextBox: true,
      lineSpacingMultiple: 1.15, valign: 'top',
    });
  });
  s.addNotes('Не границы ответственности, а предмет совместной работы с каждой функцией.');
}
...
// ============ 5. Как выстраивается взаимодействие ============
{
  ...
  ], { x: 6.45, y: 1.95, w: 6.18, h: 4.3, fontSize: 15 });
...
// ============ 7. Обратная связь и метрики ============
{
  ...
  const head = (t) => ({ text: t, options: { bold: true, fontSize: 13, color: INK } });
  const cell = (t, o = {}) => ({ text: t, options: { fontSize: 13, color: INK, ...o } });
  const rows = [
  ...
  s.addTable(rows, {
    x: ML, y: 1.85, w: 8.6, colW: [4.0, 2.7, 1.9],
    rowH: 0.42, fontFace: FONT, valign: 'middle',
    border: [
      { type: 'none' }, { type: 'none' },
  ...
  bullets(s, [
    'Журнал обращений и отзывы — регулярный разбор',
    'Тайный визит раз в год',
    'Бэклог запросов с оценкой стоимости',
  ], { x: 10.0, y: 1.95, w: 2.63, h: 3.0, fontSize: 13 });
  s.addNotes('Минимум показателей, по которым реально принимаются решения. Всё начинается с нулевого замера.');
}
...
// ============ 8. Персонал и гостеприимство ============
{
  ...
  ], { x: 5.55, y: 2.3, w: 7.08, h: 1.9, fontSize: 13 });

  s.addText('Условия', {
    x: 5.55, y: 4.35, w: 7.08, h: 0.35,
    fontFace: FONT, fontSize: 16, color: INK, bold: true, margin: 0, isTextBox: true,
  });
  bullets(s, [
  ...
  ], { x: 5.55, y: 4.8, w: 7.08, h: 1.5, fontSize: 13 });
...
// ============ 9. Стандарты и документы ============
{
  ...
  docs.forEach((d, i) => {
    ...
    s.addText(String(i + 1).padStart(2, '0'), {
      x: x + 0.22, y: 2.15, w: 1.0, h: 0.3,
      fontFace: FONT, fontSize: 12, color: MUTED, margin: 0, isTextBox: true,
    });
    s.addText(d, {
      x: x + 0.22, y: 3.55, w: dw - 0.44, h: 1.2,
      fontFace: FONT, fontSize: 14, color: INK, margin: 0, isTextBox: true,
      lineSpacingMultiple: 1.15, valign: 'bottom',
    });
  });
  s.addText('Стандарт обслуживания не пишется до аудита точек контакта', {
    x: ML, y: 5.35, w: CW, h: 0.4,
    fontFace: FONT, fontSize: 14, color: INK, italic: true, margin: 0, isTextBox: true,
  });
...
// ============ 10. Цифровые сервисы ============
{
  ...
  ], { x: ML, y: 1.95, w: 5.2, h: 2.2, fontSize: 15 });

  const pw = 1.9, pg = 0.27;
  ['Экран 1\nпоиск события', 'Экран 2\nрегистрация', 'Экран 3\nподтверждение'].forEach((c, i) => {
    photo(s, 6.4 + i * (pw + pg), 1.85, pw, 3.6, c);
  });
  s.addText('Скриншоты текущего пути с пометками, где человек теряется', {
    x: 6.4, y: 5.6, w: 6.23, h: 0.35,
    fontFace: FONT, fontSize: 11, color: MUTED, margin: 0, isTextBox: true,
  });
  s.addNotes('Скриншоты делаются самостоятельно за полчаса и работают как доказательство, а не иллюстрация.');
}
...
// ============ 12. Монетизация и продукты ============
{
  ...
  s.addText('Принцип отбора — продаём углубление, а не доступ', {
    x: ML, y: 6.3, w: CW, h: 0.4,
    fontFace: FONT, fontSize: 15, color: INK, italic: true, margin: 0, isTextBox: true,
  });
